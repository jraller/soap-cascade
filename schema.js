...
/*
function trace(name) {
	var methodRoot = schema.elements[name].children[0].children[0].children,
		childCount,
		type;

//	console.log(util.inspect(schema.elements[name], {depth: 1}));
//	console.log(schema.elements[name].children[0].children[0].children.length);

//	console.log(util.inspect(methodRoot, {depth: null}));

	for (childCount = 0; childCount < methodRoot.length; childCount++) {
//		console.log(' ', methodRoot[childCount].$name, methodRoot[childCount].$minOccurs, methodRoot[childCount].$maxOccurs, methodRoot[childCount].$nillable);
		console.log(' ', methodRoot[childCount].$name, methodRoot[childCount].$type);
		if (methodRoot[childCount].$type.substring(0, 5) === 'impl:') {
			type = methodRoot[childCount].$type.substring(5);

			console.log('- ', type, typeof type);
			if (schema.complexTypes[type]) {
				console.log(schema.complexTypes[type].children[0].children.length);
			} else {
				console.log('#################', schema.types[type].children[0].children.length);
			}

		} else {
			console.log('   ', methodRoot[childCount].$type);
		}
	}
}
*/
...
function complexType(NS, which, name) {
	...
	stub[which] = {};
	stub[which][name] = ['complexTypeStub'];
...
//				console.log('choice' + typeNS + typeName);

				if (typeNS === 'impl') {
//					console.log(util.inspect(choice, {depth: 1}));
					stub[which][name].push(complexType(typeNS, typeName, '&&&' + choice.$name));
				}
			});
		}
...
//	console.log(chapter, name);
	...
	if (schemas[chapter] && schemas[chapter][name]) {
		target = schemas[chapter][name];
	} else {
//		console.log('no target', chapter, name);
	}

	if (chapter === 'elements') {
		me = [];
		target.children[0].children[0].children.forEach(function (child) {
			childFullname = child.$type.split(':');
			childType = childFullname[0];
			childName = childFullname[1];
			if (childType === 'impl') {
				if (schemas.complexTypes[childName]) {
					childChapter = 'complexTypes';
				}
				if (schemas.types[childName]) {
					childChapter = 'types';
				}
			} else {
				childChapter = 'types';
			}
			me.push(build(childChapter, childName));
		});
	} else if (chapter === 'complexTypes') {
		me[name] = {};

//		console.log('#', chapter, name, target.name, target.children[0].nsName);
//		console.log(util.inspect(target, {depth: 2}));

		if (target.children[0].nsName === 'sequence') {
			me[name] = [];
			target.children[0].children.forEach(function (child) {
...
//						console.log('##', grandchild.$type);
						childFullname = grandchild.$type.split(':');
						childType = childFullname[0];
						childName = childFullname[1];
						if (childType === 'impl') {
							childChapter = 'unset';
							...
							me[name].push(build(childChapter, childName));
						} else {
							me[name].push(build(grandchild.nsName, grandchild.$name));
						}
					});
				} else {
					childFullname = child.$type.split(':');
...
//					}
					if (childType === 'impl') {
						...
						me[name].push(build(child.nsName, child.$name));
					}
				}
			});
		} else if (target.children[0].nsName === 'complexContent') {
//			console.log('###');
			console.log(util.inspect(target.children[0], {depth: 3}));
			me = [];
//			target.children[0].children.forEach(function (child) {
//				console.log('content', child.$base);
//				console.log(util.inspect(child, {depth: 3}));
//				me.push(build('complexTypes', child.$base));
//			});
		} else if (target.children[0].nsName === 'choice') {
			me[name] = [];
//			console.log('###');
//			console.log(util.inspect(target.children[0], {depth: null}));
			target.children[0].children.forEach(function (child) {
				console.log(util.inspect(child, {depth: null}));
//				console.log('##', grandchild.$type);
				childFullname = child.$type.split(':');
